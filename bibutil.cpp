...
void _leMateriais(string nomeArquivo, string caminho, bool mipmap) {
    ...
    while (!fp.eof()) {
        ...
        if (tokens[0] == "newmtl") { // Novo material ?
            // Se material já existe na lista, pula para o
            // próximo
            string nomeMat = tokens[1];
        ...
        if (tokens[0] == "Ka") { // Ambiente
            if (ptr==NULL) continue;
			ptr->ka[0] = atof(tokens[1].c_str());
			ptr->ka[1] = atof(tokens[2].c_str());
			ptr->ka[2] = atof(tokens[3].c_str());
        }
        if (tokens[0] == "Kd") { // Difuso
            if (ptr==NULL) continue;
            ptr->kd[0] = atof(tokens[1].c_str());
			ptr->kd[1] = atof(tokens[2].c_str());
			ptr->kd[2] = atof(tokens[3].c_str());
        }
        if (tokens[0] == "Ks") { // Especular
            if (ptr==NULL) continue;
            ptr->ks[0] = atof(tokens[1].c_str());
			ptr->ks[1] = atof(tokens[2].c_str());
			ptr->ks[2] = atof(tokens[3].c_str());
        }
        if (tokens[0] == "Ns") { // Fator de especularidade
            if (ptr==NULL) continue;
            ptr->spec = atof(tokens[1].c_str());
        ...
        if (tokens[0] == "map_Kd") { // textura
            if (ptr==NULL) continue;
			cout << "++++++++++++++++++++++" << caminho+tokens[1]<< "\n";
            texPtr = CarregaTextura(caminho+tokens[1], mipmap);
