...
struct OBJ {
	...
	OBJ *clone()
	{
		...
		{
			MESH *m = new MESH(*meshes[i]);
			clonado->meshes.push_back(m);
		}
		clonado->normais_por_vertice = normais_por_vertice;
		clonado->tem_materiais = tem_materiais;
		clonado->textura = textura;
		return clonado;
	}
};
...
void DesenhaMesh(MESH* mesh, bool temMateriais=true, bool normaisPorVertice=true, GLint textura=-1);
